...
type AsServerMaglumi800Lan struct {
	MESSAGE_ORDER    int
	MESSAGE_LENGTH   int
	MESSAGE_TYPE     int
	QUERY            int
...
func NewAsServerMaglumi800Lan(name string, apiHost string, conf tools.EquipmentConfig) *AsServerMaglumi800Lan {
	return &AsServerMaglumi800Lan{
		MESSAGE_ORDER:  0,
		host:           conf.Host,
		apiHost:        apiHost,
		eqName:         name,
...
func (d *AsServerMaglumi800Lan) Connect() {
...
func (as *AsServerMaglumi800Lan) GetConnectionStatus() bool {
	return as.connectionStatus
}

func (as *AsServerMaglumi800Lan) Start() {
	for {
		resArr := make([]byte, 200)
...
func (as *AsServerMaglumi800Lan) parseReceived() {
	defer func() {
		as.dataReceived = nil
	}()
	fmt.Println("data received", as.dataReceived)
...
func (as *AsServerMaglumi800Lan) saveResults() {
	barcode := ""
	...
	reqData := AnalizatorResultReq{
		apiHost: as.apiHost,
		Name:    as.eqName,
		Barcode: barcode,
		Items:   results,
	}
	err = saveOrder(reqData)
	if err != nil {
		fmt.Println("failed to save order", err.Error())
	}
	fmt.Println("Gotten results", string(res))
}
func (as *AsServerMaglumi800Lan) getOrder() []string {
	var order []string
	...
	res, err := getOrder(as.apiHost, as.eqName, barcode)
	if err != nil {
		as.tools.ErrorLog("failed to get order", err)
		exists = false
	}
...
func (as *AsServerMaglumi800Lan) getOrderList() [][]string {
	return nil
}

func (as *AsServerMaglumi800Lan) GetType() string {
	return "server"
}

// func handleConnection(server *AsServerMaglumi800Lan) {
...
// func Maglumi800Lan() AsServerMaglumi800Lan {
// 	server := AsServerMaglumi800Lan{
// 		// Initialize your AsServerMaglumi800Lan properties here
